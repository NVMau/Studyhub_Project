...
const Home = () => {
  ...
  const fetchHocKy = async () => {
    try {
      const response = await fetch("https://localhost:5017/api/HocKy");
  ...
  useEffect(() => {
    if (hocKy != null) setSemester(hocKy[hocKy.length - 1]);
  }, [hocKy]);
  ...
  useEffect(() => {
    const fetchCourseST = async () => {
      try {
        const response = await fetch(
          `https://localhost:7111/api/KhoaHoc/${user.userInfo.IdUser}/${semester.IdHocKy}`
        );
    ...
    const fetchCourseTC = async () => {
      try {
        const response = await fetch(
          `https://localhost:7111/api/KhoaHoc/khoaHocByGVHK/${user.userInfo.IdUser}/${semester.IdHocKy}`
        );
        const dataKhoaHoc = await response.json();
        sessionStorage.setItem("khoahoc", JSON.stringify(dataKhoaHoc));
        setCourses(JSON.parse(sessionStorage.getItem("khoahoc")));
    ...
    if (semester != null) {
      if (user.userInfo.Role === "Student") fetchCourseST();
      else fetchCourseTC();
    }
  }, [semester]);
  // hàm thay đổi học ky
  const handleSemesterChange = (event) => {
    ...
    const selectedSemester = hocKy.find(
      (hk) => hk.IdHocKy === selectedSemesterId
    ); // Tìm học kỳ tương ứng với Id đã chọn
                  ...
                  {!courses ? (
                    ...
                    <>
                      {courses.length === 0 ? (
                        ...
                        <>
                          {JSON.parse(sessionStorage.getItem("khoahoc")).map(
                            (kh) => (
                              <div
                                class="teacher btn"
                                onClick={() => courseDetail(kh)}
                              >
                                <img
                                  src="https://i.pinimg.com/564x/c9/91/8c/c9918c931842e016253fe8dd268bf35a.jpg"
                        ...
                        <>
                          {JSON.parse(sessionStorage.getItem("khoahoc")).map(
                            (kh) => (
                              ...
                              >
                                <img
                                  src="https://i.pinimg.com/564x/56/65/37/566537bb231263dd1f26f6267ed722b7.jpg"
                                  className="img-course"
                                  alt="loi"
                                />
                                <div className="info-couse">
                                  ...
                                  <p>
                                    Lớp có {kh.SoLuongSinhVien} học sinh - Số
                                    tín chỉ: {kh.SoTinChi}
                                  </p>
                                </div>
                              </div>
